   ...
   const fifteens =  inventors.filter(function(inventor) {
    ...
    /*const category = document.querySelector('.mw-category'); //wybieramy div z linkami
    const link = Array.from(category.querySelectorAll('a')); // nodelist z linkami--> trzeba zamienić na Array, bo nie działa map
    const de = link
                  .map(function(link) { // array z tytułami
                    return link.textContent;
                  })
                  .filter(function(title) {
                    return title.includes('de');
                  });*/

    // 7. sort Exercise
    // Sort the people alphabetically by last name
    const sortByName = people.sort(function(first, second) {
